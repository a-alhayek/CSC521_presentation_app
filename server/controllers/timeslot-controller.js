...
changeTimeslotStatusToTrue = (req, res) => {
...
changeTimeslotStatusToFalse = (req, res) => {
  Timeslot.findOneAndUpdate({ _id: req.params.id }, { status: false }, (err, timeslot) => {
    if (err) {
      console.error(`400 in 'changeTimeslotStatus': ${err}`);
      return res.status(400).json({
        success: false,
...
module.exports = {
  ...
  createTimeslots,
  removeTimeslot,
  changeTimeslotStatusToTrue,
  changeTimeslotStatusToFalse,
};
